...
class HealthServicesController extends Controller
{
	public function ResidentDisplay($typeofview){
		...
		if ($typeofview == "ChronicCough"){
			...
			$infant_resident = DB::table('t_resident_basic_info AS R')
	            ->leftjoin('t_hs_newborn AS NB', 'NB.RESIDENT_ID', 'R.RESIDENT_ID')
	            ->leftjoin('t_hs_infant AS I', 'I.NEW_BORN_ID', 'NB.NEWBORN_ID')
	    	...
	    	$infant_nonresident = DB::table('t_nonresident_basic_info AS NR')
	            ->leftjoin('t_hs_newborn AS NB', 'NB.NONRESIDENT_ID', 'NR.NONRESIDENT_ID')
	            ->leftjoin('t_hs_infant AS I', 'I.NEW_BORN_ID', 'NB.NEWBORN_ID')
			...
			$child_resident = DB::table('t_resident_basic_info AS R')
	            ->leftjoin('t_hs_newborn AS NB', 'NB.RESIDENT_ID', 'R.RESIDENT_ID')
	            ->leftjoin('t_hs_infant AS I', 'I.NEW_BORN_ID', 'NB.NEWBORN_ID')
	            ->leftjoin('t_hs_child AS C', 'C.INFANT_ID', 'I.INFANT_ID')
	...
	public function CRUDFamilyPlanning(Request $request){
		...
		//CHANGE TAG TO NON-USER
		else if($CRUD_STATUS == "ChangeToNonuser"){

			$count = db::table('t_hs_non_family_planning_users')->where('RESIDENT_ID',$RESIDENT_ID)->count();

			if( $count > 0 ){
			 	$updateNFP=DB::table('t_hs_non_family_planning_users')
                    ->where('RESIDENT_ID',$RESIDENT_ID)
                    ->update([ 'REASONS_NOT_USING'=> $REASONS_NOT_USING, 
                    	'DATE_OF_VISIT'=> $DATE_OF_VISIT,
				...
				$NewNFP = DB::table('t_hs_non_family_planning_users')
					->insert(array(
							'RESIDENT_ID' => $RESIDENT_ID
							,'IS_INTERESTED_IN_FP' => 0
							,'REASONS_NOT_USING' => $REASONS_NOT_USING
							,'DATE_OF_VISIT' => $DATE_OF_VISIT
			...
			$updateFromFP=DB::table('t_hs_family_planning')
                    ->where('RESIDENT_ID',$RESIDENT_ID)
                    ->update([ 'UPDATED_AT'=> Carbon::now(), 'ACTIVE_FLAG'=> 0 ]);

            return response()->json(['message' => "The tag has been changed to Non-user."]);
		}

		//CHANGE TAG TO USER
		else if($CRUD_STATUS == "ChangeToUser"){

			$count = db::table('t_hs_family_planning')->where('RESIDENT_ID',$RESIDENT_ID)->count();

			if( $count > 0 ){
			 	$updateFP=DB::table('t_hs_family_planning')
                    ->where('RESIDENT_ID',$RESIDENT_ID)
                    ->update([ 'FP_METHOD'=>$FP_METHOD, 
                    	'FP_SOURCE'=>$FP_SOURCE,
                    	'UPDATED_AT'=>Carbon::now(), 
                    	'ACTIVE_FLAG'=> 1 ]
                    ); 
				...
				$NewFP = DB::table('t_hs_family_planning')
					->insert(array(
						'RESIDENT_ID' => $RESIDENT_ID
						,'FP_METHOD' => $FP_METHOD
						,'FP_SOURCE' => $FP_SOURCE
			...
			$updateFromNFP=DB::table('t_hs_non_family_planning_users')
                    ->where('RESIDENT_ID', $RESIDENT_ID)
                    ->update([ 'UPDATED_AT'=> Carbon::now(), 'ACTIVE_FLAG'=> 0 ]);

            return response()->json(['message' => "The tag has been changed to User."]);
		}
	}
